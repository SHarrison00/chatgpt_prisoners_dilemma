...
Angular transform (using coop out of 10):

```{r}
dat$Y10 = c(4, 10, 2, 10, 10, 4, 7, 2, 10, 4, 4, 0, 10, 0, 3, 4, 10,1, 10, 10, 0, 3, 10, 3, 10, 10, 10, 3, 0, 5)
dat$Y10at <- (10^0.5)*asin(((dat$Y10)/10)^0.5)
```

Fit linear model
```{r}
model_lm_10at <- lm(Y10at ~ Treatment * Strategy, data=dat)
summary(model_lm_at)
par(mfrow=c(2,2))
plot(model_lm_at)
```
